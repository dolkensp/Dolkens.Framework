    ...
    public class EpochDateTimeConverter : DateTimeConverterBase
    {
        public override Object ReadJson(JsonReader reader, Type objectType, Object existingValue, JsonSerializer serializer)
        {
            ...
            if (reader.TokenType == JsonToken.Integer)
            {
                ...
                {
                    return DateTimeUtilities.EPOCH.AddSeconds((UInt64)reader.Value).ToLocalTime();
                }
                else
                {
                    return DateTimeUtilities.EPOCH.AddSeconds((Int64)reader.Value).ToLocalTime();
                }
            }

            if (reader.TokenType == JsonToken.Float)
            {
                return DateTimeUtilities.EPOCH.AddSeconds((Double)reader.Value).ToLocalTime();
            }
            ...
            if (reader.TokenType == JsonToken.String)
            {
                ...
                {
                    return DateTimeUtilities.EPOCH.AddSeconds(int64).ToLocalTime();
                }
                else if (UInt64.TryParse((String)reader.Value, out uint64))
                {
                    return DateTimeUtilities.EPOCH.AddSeconds(uint64).ToLocalTime();
                }
                else if (Double.TryParse((String)reader.Value, out value))
                {
                    return DateTimeUtilities.EPOCH.AddSeconds(value).ToLocalTime();
                }
                else
                {
        ...
        public override void WriteJson(JsonWriter writer, Object value, JsonSerializer serializer)
        {
            ...
            if (value is DateTime)
            {
                ...
                text = $"{(dateTime - DateTimeUtilities.EPOCH).TotalSeconds}";
            }
